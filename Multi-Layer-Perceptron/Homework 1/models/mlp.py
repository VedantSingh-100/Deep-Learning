...
class MLP4:
    def __init__(self, debug=False):
        ...
        Refer to the diagramatic view in the writeup for better understanding.
    ...
    def forward(self, A):
        ...
        for i in range(L):  # Increment by 2 to handle linear layers
            A = self.layers[i].forward(A)

            if self.debug:
                self.A.append(A)
            
            # A = self.layers[i + 1].forward(A)  # Apply activation layer in every iteration
    ...
    def backward(self, dLdA):
        """
        Refer to the pseudo code outlined in the writeup to implement backpropagation through the model.
        """
        ...
        for i in range(L-1, -1, -1):  # Start from the last layer and decrement index by 2
            dLdA = self.layers[i].backward(dLdA)
            # dLdA = self.layers[i - 1].backward(dLdA)
