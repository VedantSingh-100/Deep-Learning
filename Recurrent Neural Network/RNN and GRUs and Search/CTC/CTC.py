...
class CTCLoss(object):
    ...
    def forward(self, logits, target, input_lengths, target_lengths):
        ...
        for batch_itr in range(B):
            ...
            batch_extended_symbols, skip_connect = self.ctc.extend_target_with_blank(batch_target)
            batch_alpha = self.ctc.get_forward_probs(batch_logits, batch_extended_symbols, skip_connect)
            batch_beta = self.ctc.get_backward_probs(batch_logits, batch_extended_symbols, skip_connect)
            batch_gamma = self.ctc.get_posterior_probs(batch_alpha, batch_beta)

            self.gammas.append(batch_gamma)
            T,N = batch_gamma.shape
            
            for t in range(T):
                 for n in range(N):
                    gamma_log = batch_gamma[t,n] * np.log(batch_logits[t, batch_extended_symbols[n]])
                    total_loss[batch_itr] -= gamma_log
    ...
    def backward(self):
        ...
        for batch_itr in range(B):
            ...
            for t in range(T):
                for n in range(N):
                    dY[t, batch_itr, batch_extended_symbols[n]] -= self.gammas[batch_itr][t,n] / batch_logits[t, batch_extended_symbols[n]]
