...
# Go up one level from the current directory and then into the 'mytorch' directory
mytorch_path = os.path.abspath(os.path.join(current_dir, '..', 'mytorch'))

# Add 'mytorch' directory to the system path
sys.path.append(mytorch_path)
...
class RNNPhonemeClassifier(object):
    ...
    def forward(self, x, h_0=None):
        ...
        for i in range(seq_len):
            for layer in range(self.num_layers):
                print(f"Forward - Time Step: {i}, Layer: {layer}")
                print(f"Input Shape: {x[:, i].shape}, Hidden Shape: {hidden[layer].shape}")
                if layer == 0:
                    hidden[layer] = self.rnn[layer](x[:, i], hidden[layer])
                else:
                    hidden[layer] = self.rnn[layer](hidden[layer-1], hidden[layer])
                print(f"Updated Hidden Shape: {hidden[layer].shape}")
            self.hiddens.append(hidden.copy())
    ...
    def backward(self, delta):
        ...
        * Normalize dh by batch_size since initial hidden states are also treated
        ...
        #         if i !=0:
        #             h_prev_l = self.hiddens[i+1][layer-1]
        #         else:
        #             h_prev_l = self.x[:,i,:]
        #     h_prev_t = self.hiddens[i][layer]
        #     retdx, retdh = self.rnn[layer].backward(dh[layer], self.hiddens[i+1][layer], h_prev_l, h_prev_t)
        #     if layer>0:
        #         dh[layer-1] +=retdx
        #     else:
        #         dh[layer] = retdh        
        ...
        for i in reversed(range(seq_len)):
            for layer in reversed(range(self.num_layers)):
                print(f"Backward - Time Step: {i}, Layer: {layer}")
                if layer == 0:
                    h_prev_l = self.x[:, i]
                else:
                    h_prev_l = self.hiddens[i+1][layer-1]
                h_prev_t = self.hiddens[i][layer]
                print(f"Shapes - h_prev_l: {h_prev_l.shape}, h_prev_t: {h_prev_t.shape}, dh: {dh[layer].shape}")

                newdx, newdh = self.rnn[layer].backward(dh[layer], self.hiddens[i+1][layer], h_prev_l, h_prev_t)
                if layer > 0:
                    dh[layer-1] += newdx
                dh[layer] = newdh
        return dh / batch_size
